...
namespace Tobo.Net
{
	// https://github.com/in0finite/NetworkDiscoveryUnity
	...
	public class NetworkDiscovery : MonoBehaviour
	{
		...
		[SerializeField]
		private int discoveryPort = DefaultDiscoveryPort;
		...
		public Action<DiscoveryInfo> OnServerDiscovered = delegate { };
		...
		public const string SignatureKey = "Signature", PortKey = "Port", MapNameKey = "Map";
		public const int DefaultDiscoveryPort = 18418;
		...
		void Awake()
		{
			...
			RegisterResponseData(SignatureKey, GetCachedSignature());
			//RegisterResponseData(PortKey, this.gameServerPortNumber.ToString(CultureInfo.InvariantCulture));
			//RegisterResponseData(MapNameKey, SceneManager.GetActiveScene().name);
		}
		...
		public void InitServer()
		{
			...
			m_serverUdpCl = new UdpClient(discoveryPort);
		...
		static DiscoveryInfo ReadDataFromUdpClient(UdpClient udpClient)
		{
			...
			{
				return HandleClientData(remoteEP, receivedBytes);
			}

			return null;
		}

		static DiscoveryInfo HandleClientData(IPEndPoint remoteEP, byte[] data)
        {
			ByteBuffer buf = ByteBuffer.Get();
			buf.ReadData(data);

			ushort port = buf.Read<ushort>();
			Dictionary<string, string> dict = GetDict(buf);
			return new DiscoveryInfo(remoteEP, port, dict);
		}
		void UpdateServer()
		{
		...
		{
			if (info.KeyValuePairs.ContainsKey(SignatureKey) && info.KeyValuePairs[SignatureKey] == GetCachedSignature())
			{
				// signature matches
				// send response
				...
				ByteBuffer buf = ByteBuffer.Get();
				buf.Write(NetworkManager.Port);
				WriteDict(buf, m_responseData);
				m_serverUdpCl.Send(buf.Data, buf.Written, info.EndPoint);
				Profiler.EndSample();
			}
		}

		static Dictionary<string, string> GetDict(ByteBuffer buf)
		{
			int size = buf.GetByte();
			Dictionary<string, string> dict = new Dictionary<string, string>(size);

			for (int i = 0; i < size; i++)
			{
				dict[buf.Read()] = buf.Read();
			}

			return dict;
		}

		static void WriteDict(ByteBuffer buf, Dictionary<string, string> dict)
		{
			buf.Write((byte)dict.Count);
		...
		public static void GetDiscoveryRequestData(ByteBuffer buf)
		{
			Profiler.BeginSample("ConvertDictionaryToByteArray");
			var dict = new Dictionary<string, string>(System.StringComparer.InvariantCulture) { { SignatureKey, GetCachedSignature() } };
			//byte[] buffer = ConvertDictionaryToByteArray(dict);
			buf.Write<ushort>(0); // Blank, for space
			WriteDict(buf, dict);
			Profiler.EndSample();
		}
		...
		public void Search()
		{
			...
			foreach (var address in GetBroadcastAdresses())
			{
				...
				SendDiscoveryRequest(endPoint, buf);
			}
		...
		void SendDiscoveryRequest(IPEndPoint endPoint, ByteBuffer buf)
		{
			//if (!SupportedOnThisPlatform)
			//	return;
			...
			try
			{
				m_clientUdpCl.Send(buf.Data, buf.Written, endPoint);
			}
		...
		void OnReceivedServerResponse(DiscoveryInfo info)
		{
			...
			// invoke event
			this.OnServerDiscovered?.Invoke(info);
		...
		public static bool IsDataFromServerValid(DiscoveryInfo data)
		{
			...
			return data.KeyValuePairs.ContainsKey(SignatureKey) && data.KeyValuePairs[SignatureKey] == GetCachedSignature();
				//&& data.KeyValuePairs.ContainsKey(PortKey);
		}
		...
		public class DiscoveryInfo
		{
			...
			public DiscoveryInfo(IPEndPoint endPoint, ushort port, Dictionary<string, string> keyValuePairs)
			{
				this.EndPoint = endPoint;
				this.KeyValuePairs = keyValuePairs;
				m_timeWhenReceived = Time.realtimeSinceStartup;

				//TryGetGameServerPort(out ServerPort);
			...
			internal bool TryGetGameServerPort(out ushort port)
			{
				port = this.ServerPort;
				return true;
				//port = 0;
				//return this.KeyValuePairs.TryGetValue(PortKey, out string portString)
